...
class EximEnd_Target_Site_V2 {
    ...
    public function add_cors_support() {
        ...
        if (isset($_GET['rest_route']) && strpos($_GET['rest_route'], 'eximend') !== false) {
            $is_target_route = true;
        }
        ...
        if ($is_target_route) {
            ...
            header('X-Exim-Cors-Applied: true'); // Debug header
    ...
    public function register_routes() {
        ...
        register_rest_route($namespace, '/posts', [
            'methods' => 'GET',
            'callback' => [$this, 'get_paged_posts'],
            'permission_callback' => '__return_true'
        ]);

        register_rest_route($namespace, '/products', [
            'methods' => 'GET',
            'callback' => [$this, 'get_paged_products'],
            'permission_callback' => '__return_true'
        ]);

        register_rest_route($namespace, '/media', [
            'methods' => 'GET',
            'callback' => [$this, 'get_paged_media'],
            'permission_callback' => '__return_true'
        ]);
        
        register_rest_route($namespace, '/users', [
            'methods' => 'GET',
            'callback' => [$this, 'get_paged_users'],
            'permission_callback' => '__return_true'
        ]);

        register_rest_route($namespace, '/reviews', [
            'methods' => 'GET',
            'callback' => [$this, 'get_paged_reviews'],
            'permission_callback' => '__return_true'
        ]);
    }

    public function get_site_stats(WP_REST_Request $request) {
    ...
    private function get_all_post_data($post) {
    ...
    public function get_paged_posts(WP_REST_Request $request){
        ...
        if ($query->have_posts()) {
            foreach ($query->get_posts() as $post) {
                $data[] = $this->get_all_post_data($post);
            }
        }
        wp_reset_postdata();
    ...
    public function get_paged_products(WP_REST_Request $request){
        ...
        if ($query->have_posts()) {
            foreach ($query->get_posts() as $post) {
                ...
                $post_data = $this->get_all_post_data($post);
    ...
    public function get_paged_media(WP_REST_Request $request){
        $per_page = $request->get_param('per_page') ? (int) $request->get_param('per_page') : 20;
        $page = $request->get_param('page') ? (int) $request->get_param('page') : 1;
    ...
    public function get_paged_users(WP_REST_Request $request) {
        $per_page = $request->get_param('per_page') ? (int) $request->get_param('per_page') : 20;
        $page = $request->get_param('page') ? (int) $request->get_param('page') : 1;
    ...
    public function get_paged_reviews(WP_REST_Request $request) {
        $per_page = $request->get_param('per_page') ? (int) $request->get_param('per_page') : 20;
        $page = $request->get_param('page') ? (int) $request->get_param('page') : 1;
