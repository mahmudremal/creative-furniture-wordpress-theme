...
$recent_orders = $sellerDash->get_seller_recent_orders($user_id, 10);
$products = $sellerDash->get_seller_products($user_id);
...
<div class="space-y-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-white-400 flex items-center space-x-4 transition-all duration-300 hover:shadow-md">
            <div class="w-14 h-14 rounded-xl bg-secondary flex items-center justify-center shrink-0">
        ...
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-white-400 flex items-center space-x-4 transition-all duration-300 hover:shadow-md">
            <div class="w-14 h-14 rounded-xl bg-accent flex items-center justify-center shrink-0">
            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-white-400">
                    ...
                    <tr class="hover:bg-white-100 transition-colors">
                        <td class="px-6 py-4">
                            <a href="<?php echo esc_url($order->get_view_order_url()); ?>" class="text-black font-semibold hover:underline">
                                #<?php echo $order->get_order_number(); ?>
                            </a>
                        </td>
                        <td class="px-6 py-4 text-black"><?php echo esc_html($product->get_name()); ?></td>
                        <td class="px-6 py-4 text-center text-black"><?php echo $item->get_quantity(); ?></td>
                        <td class="px-6 py-4 text-black"><?php echo wc_price($item->get_total()); ?></td>
                        <td class="px-6 py-4">
                            <?php 
                            $status = $order->get_status();
                            $status_classes = 'px-3 py-1 rounded-full text-xs font-medium ';
                            if ($status === 'completed') $status_classes .= 'bg-green-100 text-green-700';
                            elseif ($status === 'processing') $status_classes .= 'bg-blue-100 text-blue-700';
                            elseif ($status === 'pending') $status_classes .= 'bg-yellow-100 text-yellow-700';
                            else $status_classes .= 'bg-gray-100 text-gray-700';
                            ?>
                            <span class="<?php echo $status_classes; ?>">
                                <?php echo esc_html(wc_get_order_status_name($status)); ?>
                            </span>
                        </td>
                        <td class="px-6 py-4 text-white-800"><?php echo $order->get_date_created()->date_i18n('M d, Y'); ?></td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
